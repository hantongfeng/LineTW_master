...
class Data:
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, edge_ids: np.ndarray, labels: np.ndarray,
        max_ids_list: np.ndarray, max_src_label_list: np.ndarray, max_dst_label_list: np.ndarray):
        """
        ...
        self.max_ids_list=max_ids_list
        self.max_src_label_list=max_src_label_list
        self.max_dst_label_list=max_dst_label_list

    def save_data(self, folder_name: str):
        folder_path = f"./processed_data/{folder_name}"

        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
            print(f"Directory {folder_path} created.")
        else:
            print(f"Directory {folder_path} already exists. Extracting data...")

            # Load data if it already exists
            self.src_node_ids = np.load(os.path.join(folder_path, 'src_node_ids.npy'))
            self.dst_node_ids = np.load(os.path.join(folder_path, 'dst_node_ids.npy'))
            self.node_interact_times = np.load(os.path.join(folder_path, 'node_interact_times.npy'))
            self.edge_ids = np.load(os.path.join(folder_path, 'edge_ids.npy'))
            self.labels = np.load(os.path.join(folder_path, 'labels.npy'))
            self.max_ids_list = np.load(os.path.join(folder_path, 'max_ids_list.npy'))
            self.max_src_label_list = np.load(os.path.join(folder_path, 'max_src_label_list.npy'))
            self.max_dst_label_list = np.load(os.path.join(folder_path, 'max_dst_label_list.npy'))
            return

        # Save data if folder was newly created
        np.save(os.path.join(folder_path, 'src_node_ids.npy'), self.src_node_ids)
        np.save(os.path.join(folder_path, 'dst_node_ids.npy'), self.dst_node_ids)
        np.save(os.path.join(folder_path, 'node_interact_times.npy'), self.node_interact_times)
        np.save(os.path.join(folder_path, 'edge_ids.npy'), self.edge_ids)
        np.save(os.path.join(folder_path, 'labels.npy'), self.labels)
        np.save(os.path.join(folder_path, 'max_ids_list.npy'), self.max_ids_list)
        np.save(os.path.join(folder_path, 'max_src_label_list.npy'), self.max_src_label_list)
        np.save(os.path.join(folder_path, 'max_dst_label_list.npy'), self.max_dst_label_list)
...
class NeighborSampler: # 邻居节点采样器
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            # additional for time interval aware sampling strategy (proposed in CAWN paper) 附加的时间间隔感知采样策略
            if self.sample_neighbor_strategy == 'time_interval_aware':
        ...
        # print("&&&&&&&&&&&&&&&&&&&&&&&&&",len(self.nodes_neighbor_ids))# 有9179和9228两个值
        # print("&&&&&&&&&&&&&&&&&&&&&&&&&self.nodes_neighbor_ids[16]",self.nodes_neighbor_ids[16])
        # print("&&&&&&&&&&&&&&&&&&&&&&&&&self.nodes_neighbor_ids[1600]",self.nodes_neighbor_ids[1600])
        if self.seed is not None:
            self.random_state = np.random.RandomState(self.seed)

    def compute_sampled_probabilities(self, node_neighbor_times: np.ndarray):
        """
        compute the sampled probabilities of historical neighbors based on their interaction times 根据历史邻居的交互时间计算它们的采样概率
        ...
        return sampled_probabilities # 采样概率
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        ...
        i = np.searchsorted(self.nodes_neighbor_times[node_id], interact_time)# 在数组a中插入数组v（并不执行插入操作），返回一个下标列表，这个列表指明了v中对应元素应该插入在a中那个位置上
    ...
    def find_neighbors_after(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        """
        extracts all the interactions happening before interact_time (less than interact_time) for node_id in the overall interaction graph
        the returned interactions are sorted by time. 
        :param node_id: int, node id
        ...
        # return index i, which satisfies list[i - 1] < v <= list[i]
        # return 0 for the first position in self.nodes_neighbor_times since the value at the first position is empty
        i = np.searchsorted(self.nodes_neighbor_times[node_id],
                            interact_time)  
...
def get_neighbor_sampler(data: Data, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    ...
    #print("adj_list[100]",adj_list[100])
...
class NegativeEdgeSampler(object): # 负样本采样器
