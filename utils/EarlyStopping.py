...
class EarlyStopping(object):
    ...
    def save_checkpoint(self, model: nn.Module):
        ...
        state_dicts = {
        'model0': model[0].state_dict(),
        'model1': model[1].state_dict()
    }
        torch.save(state_dicts, self.save_model_path)
        if self.model_name in ['JODIE', 'DyRep', 'TGN']:
    ...
    def load_checkpoint(self, model: nn.Module, map_location: str = None):
        ...
        state_dicts = torch.load(self.save_model_path, map_location=map_location)
        model[0].load_state_dict(state_dicts['model0'])
        model[1].load_state_dict(state_dicts['model1'])
        if self.model_name in ['JODIE', 'DyRep', 'TGN']:
