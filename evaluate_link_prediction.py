...
if __name__ == "__main__":
    ...
    new_node_test_idx_data_loader = get_idx_data_loader(indices_list=list(range(len(new_node_test_data.src_node_ids))), batch_size=args.batch_size, shuffle=False)
        ...
        for run in range(args.num_runs):
            ...
            dynamic_backbone = LineTW(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=full_neighbor_sampler,
            ...
            bayes_model = def_complete_model(input_dim=1,hidden_dim=node_raw_features.shape[1],output_dim=1)
            ...
            early_stopping.load_checkpoint(models, map_location='cpu')

            model = convert_to_gpu(models[0], device=args.device)
            bayes_model=convert_to_gpu(models[1], device=args.device)
