...
class LineTW(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        :param max_input_sequence_length: int, maximal length of the input sequence for each node #这个又是几个意思
        :param device: str, device
        """
        super(LineTW, self).__init__()
    ...
    def pad_sequences(self, node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: list, nodes_edge_ids_list: list,
        ...
        for idx in range(len(nodes_neighbor_ids_list)):
            ...
            if len(nodes_neighbor_ids_list[idx]) > max_input_sequence_length - 1:# 某目标节点的邻居节点的长度，输入序列的长度
                # cut the sequence by taking the most recent max_input_sequence_length interactions 通过获取最近的max_input_sequence_length交互来切割序列
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        # ndarray, set the time features to all zeros for the padded timestamp 将填充时间戳的时间特征设置为全零
    ...
    def get_patches(self, padded_nodes_neighbor_node_raw_features: torch.Tensor, padded_nodes_edge_raw_features: torch.Tensor,
        ...
        """
        get the sequence of patches for nodes 获取节点的补丁序列
...
class NeighborCooccurrenceEncoder(nn.Module):     # 邻居节点共现特征编码
    ...
    def count_nodes_appearances(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        for src_padded_node_neighbor_ids, dst_padded_node_neighbor_ids in zip(src_padded_nodes_neighbor_ids, dst_padded_nodes_neighbor_ids):
            ...
            # return_index：如果为
            # true，返回新列表元素在旧列表中的位置（下标），并以列表形式存储。
            # return_inverse：如果为true，返回旧列表元素在新列表中的位置（下标），并以列表形式存储。
            # return_counts：如果为
            # true，返回去重数组中的元素在原数组中的出现次数。


            # 我们可以使用src_unique_keys[src_inverse_indices]来重建原始输入，并使用src_counts[src_inverse_indices]来获取原始输入的计数
