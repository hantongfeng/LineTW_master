...
class def_complete_model(nn.Module):
    ...
    def forward(self, z,x,y):  # 直接接收 [B, num_hops]
        x = torch.cat([x, y], dim=0).T
        x = self.act(self.fc1(x))
        x = self.act(self.fc2(x))
        z=self.act(self.fc_a(z))
        x2 = torch.cat([z, x], dim=1)
        return self.fc3(x2) 
...
class Mybayes(nn.Module):

    def __init__(self, input_dim1: int, input_dim2: int,input_dim3: int, hidden_dim: int, output_dim: int,bayes_num:int):
        """
        Merge Layer to merge two inputs via: input_dim1 + input_dim2 -> hidden_dim -> output_dim.
        :param input_dim1: int, dimension of first input
        :param input_dim2: int, dimension of the second input
